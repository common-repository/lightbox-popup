...
class wpdevart_lightbox_front_end{
	...
	public function generete_custom_css(){
		...
		#wpdevart_lb_information_content{
			width:100%;	
			padding-top:'.$this->params["information_panel_padding_top"].'px;
			padding-bottom:'.$this->params["information_panel_padding_bottom"].'px;
		}
	...
	/*###################### A function that adds a class for content images ##################*/		
	
	public function add_class_to_content_image($content){
		global $post;
		if(isset( $post->ID ) &&  $post->ID)
			$id=$post->ID;
			//Adding URL attribute for the Lightbox Popup plugin
			...
			if($this->params["enable_video_popuping"]=="enable"){
				$content=preg_replace_callback($pattern, function($all){
					$loc_pattern="/(.*)?rel=['\"](.*?)['\"](.*)/i";
					$loc_content=$all[1];
					if(preg_match($loc_pattern,$loc_content)){
